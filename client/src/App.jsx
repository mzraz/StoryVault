...
function App() {
  ...
  useEffect(()=>{
    ...
    dispatch(profileImg({imgUrl: imageurl, firstN:firstName, lastN:lastName}))
    ...
    <GoogleOAuthProvider clientId="1006775000308-06t6qt89v91v4lht65tbg2i4sq8d919u.apps.googleusercontent.com">
      ...
      <Auth>
        ...
        }
        {
          ...
          {localStorage.getItem('jwtToken') ?
            (
              localStorage.getItem('status') == "false" ?
                ...
                :
                <>
                  <Route exact path="/book" element={<BookSetupPage setLoading={setLoading} />} />
                  <Route exact path="/dashboard" element={<ProfileDashboard setLoading={setLoading}/>} />
                  <Route exact path="/chapterselection/book/:id" element={<ChapterSelection />} />
                  <Route exact path="/contributor/:id/:bookId" element={<Portal />} />
                  <Route exact path="/contribution/:bookId" element={<ContributionPortal />} />
                  <Route exact path="/collaborationVault" element={<CollaborationVault />} />
                  <Route exact path="/directory/:bookId" element={<Directory setLoading={setLoading}/>} />
                  <Route exact path="/reminders" element={<Reminders />} />
                  <Route exact path="/announcements" element={<Announcements />} />
                  <Route exact path="/storyvault" element={<Storyvault />} />
                  <Route exact path="/story/:bookId" element={<BookDashboard />} />
            ...
            <>
              ...
              <Route exact path="/otp/:username/:userSub" element={<OtpConfirmation />} />
              <Route exact path="/passwordCreated" element={<NewPasswordSuccess />} />
            </>
          }
          <Route path="*" element={<Navigate replace to="/" />} />
          ...
          {/* below for admin */}
          {/* { ( !localStorage.getItem('jwtToken') && !localStorage.getItem("admin_token") ) &&
                <Route exact path="/login" element={ <RegistrationPage/> } />
          } */}
          ...
          <Routes>
            ...
            <Route exact path="/giftVoucher" element={<Voucher admin={true} />} />
