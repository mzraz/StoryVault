...
const userBookSlice = createSlice({
    ...
    reducers: {
        ...
        updateAnswer: (state, action) => {
            ...
            const chapterIndex = state.bookData.findIndex(chapter => chapter.id === chapterId);
            if (chapterIndex !== -1) {
                const questionIndex = state.bookData[chapterIndex].questions.findIndex(question => question.id === questionId);
                if (questionIndex !== -1) {
        ...
        removeCustomChapter: (state, action) => {
            ...
            let updated= state.bookData.filter(chapter => chapter.id !== chapterIdToDelete);
        ...
        removeCustomQuestion: (state, action) => {
            ...
            // Find the chapter by ID
            const chapterToUpdate = state.bookData.find(chapter => chapter.id === chapterId);
...
export const { setBook, updateAnswer, openChapter, createCustomChapter, removeCustomChapter, createCustomQuestion, removeCustomQuestion, uploadImage, cropImg } = userBookSlice.actions;
