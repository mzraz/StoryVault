...
function StoryBook({ bookData }) {
    ...
    useEffect(() => {
        if (bookData?.title) {
            ...
            const frontImageUrl = `https://s3.us-west-2.amazonaws.com/www.mystoryvault.co/${process.env.REACT_APP_S3_BUCKET}/bookcovers/${bookData?.title}front${awsId?.sub}.png`
            const backImageUrl = `https://s3.us-west-2.amazonaws.com/www.mystoryvault.co/${process.env.REACT_APP_S3_BUCKET}/bookcovers/${bookData?.title}back${awsId?.sub}.png`
    ...
    const getBook = async () => {
        await axios
            .get(`${process.env.REACT_APP_API_BASE_URL}/file/url/${bookData?.title}${awsId?.sub}.pdf`)
            .then((res) => {
                setBookUrl(res.data.fileUrl);
            })
            .catch((err) => {
                console.log(err);
            });
    ...
    const mergeCoverPages = async () => {
        ...
        html2canvas(element, { useCORS: true, allowTaint: true },{ scale: 5 }).then((canvas) => {
            canvas.toBlob(async (blob) => {
                setOpen(false)
                // Create FormData and append the Blob
                const formData = new FormData();
                formData.append('awsUserId', `final${awsId.sub}`);
    ...
    return (
        <div>
            ...
            {toggle &&
                ...
                >
                    <h1 className='text-xl font-bold text-teal-500 shadow-2xl bg-white p-10 rounded-md'>Creating Book Cover...</h1>
                </div>
            }
            <div>
                                ...
                                >
                                    ...
                                    {/* 2nd for custom */}
                                    {
                                        !(bookData.template_id != 0 && bookData.cover_template_id == 3) ?
                                            <div
                                                className={`flex flex-col justify-center ${bookData.cover_template_id == 0 ? "ml-2" : "-ml-2"}`}
                                                style={{ width: `${spineWidthInches}in`, backgroundColor: bookData?.cover_template_id === 0 ? 'white' : bookData?.book_color, height: 864 }}
                                            >
                                                {!(bookData.template_id != 0 && bookData.cover_template_id == 3) &&
                                                    bookData.title.split('').map((d, index) => (
                                                        <span key={index} style={{color:bookData?.spine_color}} className="text-center font-semibold pl-3.5 mt-1 p-0 h-2 transform rotate-90 origin-right">
                                                            {""/*d*/}
                                                        </span>
                                                    ))
                                                }
                                            ...
                                            >
                                                {customSpine && 
                                                    bookData.title.split('').map((d, index) => (
                                                        <span key={index} style={{color:bookData?.spine_color}} className="text-center font-semibold p-0 mt-1 h-2 transform rotate-90 origin-right">
                                                           {""/*d*/} 
                                                        </span>
                                                    ))
                                                }
            ...
            <Document file={bookurl} onLoadSuccess={onDocumentLoadSuccess} className="docPage">
                <PageFlip ref={pageFlipRef} onFlip={onFlip} width={width} height={height}>
                    {Array.from(new Array(numPages), (el, index) => (
                        <Page key={`page_${index + 1}`} width={width} className="" pageNumber={index + 1} />
                    ))}
                </PageFlip>
            </Document>
            ...
            <div className="mt-6">
                <nav className="border-t border-gray-200 px-4 flex items-center justify-between sm:px-0">
                    ...
                    <div className="-mt-px w-0 flex-1 flex justify-end">
                        <button
                            disabled={currentPage === (numPages % 2 === 0 ? numPages - 2 : numPages - 1)}
                            onClick={handleNext}
                            className={classNames(
                                currentPage === (numPages % 2 === 0 ? numPages - 2 : numPages - 1)
                                    ? 'text-gray-200 hover:cursor-not-allowed'
                                    : 'text-gray-500 hover:text-gray-700  hover:border-gray-300  hover:cursor-pointer',
                                'border-t-2 border-transparent pt-4 pr-1 inline-flex items-center text-sm font-medium '
                            )}
