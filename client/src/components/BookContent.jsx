...
export default function BookContent({ bookID, content, setContent, currentChapter, stories, bookData }) {
  ...
  const [pageAContent, setPageAContent] = useState(pageA ? pageA.stories.ops[0].insert : undefined);
  const [pageBContent, setPageBContent] = useState(pageB ? pageB.stories.ops[0].insert : undefined);
  ...
  }, [content, pageA, pageB, currentChapter, pageAContent, pageBContent, pageIndex]);
  ...
  const addNewStory = async () => {
    ...
    await API.addStory(bookID, content[bookState.addStory.page]._id, bookState.text).then((res) => {
  ...
  const handleStoryEdit = (story, page) => {
    dispatch(setEditStory({ show: !bookState.editStory.show, id: story._id, page: page }));
  ...
  useEffect(() => {
    ...
    const handler = (delta, oldDelta, source) => {
      ...
      API.editPage(content[pageClicked + pageIndex]._id, quill.getContents()).then((res) => {
        if (pageClicked === 0) {
          document.getElementById('pageA').innerText = res.data.stories.ops[0].insert;
          //setPageAContent(res.data.stories.ops[0].insert);
        } else {
          document.getElementById('pageB').innerText = res.data.stories.ops[0].insert;
  ...
  const saveEditor = async () => {
    console.log('content[pageClicked + pageIndex]._id => ', content[pageClicked + pageIndex]._id);
    await API.editPage(content[pageClicked + pageIndex]._id, quill.getContents()).then((res) => {
  ...
  return (
    <>
      {visibility ? <AppAlert onCancel={confirmAlert} onConfirm={cancelAlert} /> : null}
      ...
      <div className="bg-white page-container  shadow sm:rounded-lg ">
        ...
        <div className="px-4 py-5 sm:px-2">
          <div className="flex justify-between -mb-1 ">
            <div
              className={classNames(
                bookState.addStory.page === 1 && bookState.addStory.show ? 'invisible' : 'visible',
                'add-Story-page-1 flex justify-center items-center mb-2'
              )}
            >
              <button onClick={getPageCount} className="text-white bg-teal-600 h-8 rounded-md px-3 py-1.5 text-xs sm:text-base w-28 sm:w-40">
                Send to print
              </button>
            ...
            <div
              className={classNames(
                bookState.addStory.page === 0 && bookState.addStory.show ? 'invisible' : 'visible',
                'add-Story-page-2'
              )}
            >
          ...
          {/* Content goes here */}
          <div className="h-96">
            <StoryBook stories={stories} bookData={bookData} />
          </div>
        </div>
      </div>
                ...
                <div className="relative w-full max-w-2xl max-h-full mx-auto">
                    <div className="relative bg-white rounded-lg shadow">
                        ...
                        <ul className="p-6 space-y-6 space-x-4 list-disc">
                            ...
                            Hooray! You've completed your book! Before sending it for printing or creating a PDF copy, be sure to utilize the Grammarly feature embedded in the text editor. Suggested changes will be highlighted in red. This will help you polish up your content.
                            </li>
                            <li className="text-base leading-relaxed text-gray-500 ">
                            Take a good look at your book cover to catch any last-minute changes.
                            </li>
                            <li className="text-base leading-relaxed text-gray-500 ">
                            Carefully review every page to ensure proper formatting and photo placement.
                            </li>
                            <li className="text-base leading-relaxed text-gray-500 ">
                            Give yourself a big pat on the back for a job well done! You've written a one-of-a-kind keepsake that will be cherished for generations to come.
                            </li>
