...
export default function PrintBook() {
    ...
    useEffect(() => {
        if (shippingServices) {
            setSelectedShipping(shippingServices.find((data) => data.level === formData.shipping_options))
        }
    ...
    useEffect(() => {
        const shippingOptionsDisable = (formData.city && formData.country_code && formData.postcode.length >= 5 && formData.state_code && formData.street && formData.quantity)
        if (shippingOptionsDisable) {
            getShippingOptions();
        }
    }, [formData.city, formData.country_code, formData.postcode, formData.state_code, formData.street, formData.quantity])
    ...
    const getShippingOptions = async () => {
        ...
        await axios({
            ...
            .then((res) => {
                ...
                const freeService = res.data.find((d) => d.carrier_service_name == "UPS Mail Innovation")
                setShippingServices(res.data);
                if (freeService) {
                    setFormData((prevState) => ({ ...prevState, shipping_options: freeService.level }))
                }
            })
            .catch((err) => {
                console.log(err);
    ...
    const getUserStatus = async () => {
        await axios({
            method: 'GET',
            url: `${process.env.REACT_APP_API_BASE_URL}/api/get/free_book/${userDetails.sub}`
        }).then((res) => {
            console.log(res)
    ...
    const getCost = async () => {
        await axios({
            ...
            data: {
                ...
                bookId: state.bookId,
                quantity: formData.quantity,
                shipping_charges: formData.shipping_options === 'MAIL' ? 0 : selectedShipping?.cost_excl_tax
            }
        }).then((res) => {
            console.log(res)
    ...
    const createPaymentIntent = async () => {
        try {
            const response = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/api/checkout/stripePayment`, { amount: discountPrice });
    ...
    const stepForward = async () => {
        if (discountPrice === 0 && step === 2) {
            ...
            axios.post(`${process.env.REACT_APP_API_BASE_URL}/api/upload/luluCover`,body).then((res)=>{
                setCoverUrl(res?.data?.coverUrl)
                setBookUrl(res?.data?.storyUrl)
                createPrintJob(res?.data?.coverUrl, res?.data?.coverUrl);
            }).catch(err=>{
    ...
    const disabled = !(formData.city && formData.country_code && formData.phone_number?.length >= 10 && formData.postcode.length >= 5 && formData.state_code && formData.street && formData.quantity && formData.shipping_options)
    const shippingOptionsDisable = !(formData.city && formData.country_code && formData.postcode.length >= 5 && formData.state_code && formData.street && formData.quantity)
        ...
        return (
            <>
                <div className='w-full h-full'>
                    <div style={{ minHeight: '260px' }} className='w-1/2 bg-white p-4 rounded-lg mx-auto my-28'>
                        <PaymentElement />
                    </div>
                    <div className="flex justify-end space-x-5 mt-10">
                        <button onClick={stepBack} className='bg-[#DDC5BF] text-white font-bold py-2 px-4 rounded-lg'>
                            Back
                        </button>
                ...
                <div className="w-full min-h-screen bg-[#F3F4F6] p-6">
                    ...
                    {step === 1 ?
                        <div className="w-full h-5/6">
                            <h3 className="font-semibold text-2xl font-sans text-left mb-4">Shipping Details</h3>
                            <div className="">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 ">
                                    <div>
                                        ...
                                        />
                                        {formErrors.name && (
                                            <h1 className="text-red-500 text-sm">{formErrors.name}</h1>
                                        )}
                                        ...
                                        {formErrors.name && (
                                            <h1 className="text-red-500 text-sm">{formErrors.country_code}</h1>
                                        )}
                                        ...
                                        {formErrors.name && (
                                            <h1 className="text-red-500 text-sm">{formErrors.city}</h1>
                                        )}
                                        ...
                                        />
                                        {formErrors.name && (
                                            <h1 className="text-red-500 text-sm">{formErrors.phone_number}</h1>
                                        )}
                                        ...
                                        />
                                        {formErrors.name && (
                                            <h1 className="text-red-500 text-sm">{formErrors.quantity}</h1>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        />
                                        {formErrors.name && (
                                            <h1 className="text-red-500 text-sm">{formErrors.email}</h1>
                                        )}
                                        ...
                                        {formErrors.name && (
                                            <h1 className="text-red-500 text-sm">{formErrors.state_code}</h1>
                                        )}
                                        ...
                                        />
                                        {formErrors.name && (
                                            <h1 className="text-red-500 text-sm">{formErrors.street}</h1>
                                        )}
                                        <label
                                            className="block text-gray-600 text-sm font-bold mt-5 mb-2"
                                            htmlFor="postcode"
                                        ...
                                        />
                                        {formErrors.name && (
                                            <h1 className="text-red-500 text-sm">{formErrors.postcode}</h1>
                                        )}
                                        ...
                                        {formData.shipping_options && (
                                            <h1 className="text-gray-500 text-sm font-medium">{formData.shipping_options === 'MAIL' ? '' : `Shipping cost will be $${selectedShipping && Math.ceil(selectedShipping?.cost_excl_tax)}`}</h1>
                                        )}
                                        {formErrors.name && (
                                            <h1 className="text-red-500 text-sm">{formErrors.shipping_options}</h1>
                                        )}
                                    </div>
                                </div>
                                {error &&
                                    <h1 className='text-sm text-red-600'>{error}</h1>
                                }
                            </div>

                            <div className="flex justify-end space-x-5 mt-10">
                                <button
                                    className={`${disabled ? 'bg-gray-500 cursor-not-allowed' : 'bg-[#124E5B] cursor-pointer'} text-white  font-bold py-2 px-4 rounded-lg w-48 focus:outline-none focus:shadow-outline`}
                                    onClick={() => {
                                        getCost();
                                    }}
                                    type="button"
                                        ...
                                        <div className="w-full">
                                            ...
                                            <div className="mb-4">
                                                <div className="flex justify-between items-center">
                                                    <p className="font-semibold text-gray-600">Book Name:</p>
                                            ...
                                            <div className="mb-4">
                                                <div className="flex justify-between items-center">
                                                    <p className="font-semibold text-gray-600">Book Charges:</p>
                                                    <p className="text-gray-800">${(isNaN(actualCharges) ? 0 : actualCharges)}</p>
                                                </div>
                                            </div>
                                            <div className="mb-4">
                                                <div className="flex justify-between items-center">
                                                    <p className="font-semibold text-gray-600">Shipping Charges:</p>
                                                    <p className="text-gray-800">${shippingServices && `${selectedShipping.level === 'MAIL' ? 0 : Math.ceil(selectedShipping?.cost_excl_tax)}`}</p>
                                                </div>
                                            </div>
                                            <div className="mb-4">
                                                <div className="flex justify-between items-center">
                                                    <p className="font-semibold text-gray-600">Subtotal:</p>
                                                    <p className="text-gray-800">${isNaN(Math.ceil(selectedShipping?.cost_excl_tax) + cost) ? 0 : cost}</p>
                                                </div>
                                            </div>
                                            <div className="mb-4">
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    <div className='flex space-x-2'>
                                                        <input
                                                            className={`shadow bg-[#F0F2F6] appearance-none border rounded w-28 py-1 px-2 leading-tight focus:outline-none focus:shadow-outline ${giftBtnDisable ? 'text-gray-400 cursor-not-allowed' : 'text-black cursor-pointer'}`}
                                                            id="voucher"
                                                            type="text"
                                                            name="voucher"
                                                            minLength={7}
                                                        ...
                                                        />
                                                        <button onClick={getDiscount} disabled={voucherCode.length !== 7 || giftBtnDisable} className={`${voucherCode?.length !== 7 || giftBtnDisable ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#134f5c] cursor-pointer'} text-white text-sm font-medium py-1 px-2 rounded-md`}>
                                                            Apply
                                                        </button>
                                                    </div>
                                                </div>

                                                {<p className={`h-3 ${message?.includes('Voucher code') ? 'text-green-600' : 'text-red-600'}`}>{message}</p>}
                                            </div>
                                            ...
                                            <div className="mb-4">
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    <div className='flex space-x-2'>
                                                        <input
                                                            className={`shadow bg-[#F0F2F6] appearance-none border rounded w-28 py-1 px-2 leading-tight focus:outline-none focus:shadow-outline ${isApplied ? 'text-gray-400 cursor-not-allowed' : 'text-black cursor-pointer'}`}
                                                            id="voucher"
                                                            type="text"
                                                            name="voucher"
                                                            minLength={3}
                                                            ...
                                                            disabled={isApplied}
                                                        />
                                                        <button onClick={getPromoDiscount} disabled={promoCode.length < 3 || isApplied} className={`${promoCode?.length < 3 || isApplied ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#134f5c] cursor-pointer'} text-white text-sm font-medium py-1 px-2 rounded-md`}>
                                                            Apply
                                                        </button>
                                                    </div>
                                                </div>

                                                {<p className={`h-3 ${promoApplied?.includes('Promo code') ? 'text-green-600' : 'text-red-600'}`}>{promoApplied}</p>}
                                            </div>
                                            <hr className="border-t-2 border-gray-300 my-4" />

                                            <div className="mb-2">
                                                <p className="text-lg font-semibold text-right text-gray-800">
                                                    Total Charges: <span className="text-teal-800">${discountPrice}</span>
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="w-full h-5/6 flex flex-col">
                                    <div className="relative flex flex-col justify-center items-center w-1/2 h-2/3 shadow-xl bg-white rounded-lg p-10 mx-auto my-20">
                                        ...
                                        <div className="mb-4 ">
                                            ...
                                            <div className="flex justify-between  items-center">
                                                <p className="font-bold text-gray-600">Estimated Arrival Time: </p>
                                                <p className=" font-semibold text-gray-800 ml-8 ">{`${selectedShipping?.total_days_min} - ${selectedShipping?.total_days_max}`} days</p>
                                            </div>
                                        </div>
                                        <div className="mb-4 w-full">
                                            <label className="font-bold text-gray-600 block">Book Cover:</label>
                                            <div className="flex justify-between w-full">
                                                <div>
                                                    <input
                                                        disabled
                                                        value={coverUrl}
                                                        type="text" className="bg-[#F0F2F6] appearance-none border rounded-md w-auto xl:w-80 py-1 px-2 text-gray-400 leading-tight focus:outline-none focus:shadow-outline" />
                                                </div>
                                                <div className="flex items-center space-x-2">
                                                    ...
                                                    <FaCloudDownloadAlt
                                                        ...
                                                        color={clickedIcons.downloadCover ? '#0e4f5c' : '#6A9190'}
                                        ...
                                        <div className="w-full">
                                            <label className="font-bold text-gray-600 block">Book Interior:</label>
                                            <div className="flex justify-between w-full">
                                                <div>
                                                    <input
                                                        disabled
                                                ...
                                                <div className="flex items-center space-x-2">
                                                    <FaCopy
                                                        ...
                                                        color={clickedIcons.copyInterior ? '#0e4f5c' : '#9fc8b8'}
                                                        className="cursor-pointer w-7 h-6"
                                                    />
                                                    <FaCloudDownloadAlt
                                                        onClick={handleDownloadInterior}
                                                        color={clickedIcons.downloadInterior ? '#0e4f5c' : '#6A9190'}
                                                        className="cursor-pointer w-7 h-7"
                                                    />
                                                </div>
                                            </div>
